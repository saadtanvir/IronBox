...
Future<Stream<YoutubeVideo>> getIronBoxVideos(int skip, int take) async {
  ...
  try {
    ...
    print("Youtube Video Repo Socket Exception: ");
...
Future<Stream<YoutubeVideo>> searchYoutubeVideo(
    {@required String name, @required int skip, @required int take}) async {
  Uri uri = Helper.getUri('search_vedio_library');
  ...
  print("URI For Searching IronBox Videos: ${uri.toString()}");
  try {
    final client = new http.Client();
    // client.po
    ...
    return streamedRest.stream
        ...
        .expand((data) => (data as List))
        .map((data) {
          print("printing videos search data");
          print(data);
          return YoutubeVideo.fromJSON(data);
        });
  } on SocketException {
